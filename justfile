...
initialize:
    ...
    gh repo create Netbeheer-Nederland/dp-nbnl-equipment \
        --description "Information models for the NBNL Equipment data product." \
        --public \
        --disable-wiki

    git init -b main

    git remote add origin git@github.com:Netbeheer-Nederland/dp-nbnl-equipment.git
    git fetch
    ...
    gh repo edit Netbeheer-Nederland/dp-nbnl-equipment --default-branch v0
    git branch -D main
    ...
    #  -H "Accept: application/vnd.github+json" \
    #  -H "X-GitHub-Api-Version: 2022-11-28" \
    #  /repos/Netbeheer-Nederland/dp-nbnl-equipment/actions/permissions \
    #   -F "enabled=true" -f "allowed_actions=all"

    echo "… OK."
    echo
    echo "A GitHub repository has been created and configured at: https://github.com/Netbeheer-Nederland/dp-nbnl-equipment"
    echo
...
_post-process-linkml-schema:
    ...
    cp "information_models/dp_nbnl_equipment.schema.linkml.yml" "artifacts/information_models/"
    @echo
    @echo "Setting version in LinkML schema…"
    @echo
    sed -i '/^version: .*$/d' "artifacts/information_models/dp_nbnl_equipment.schema.linkml.yml"
    @if [ -z ${VERSION:-} ]; then \
        sed -i "/^name: .*$/a version: {{shell(ref_name)}}" "artifacts/information_models/dp_nbnl_equipment.schema.linkml.yml"; \
    else \
        sed -i "/^name: .*$/a version: ${VERSION}" "artifacts/information_models/dp_nbnl_equipment.schema.linkml.yml"; \
    fi
    @echo "… OK."
    @echo
...
[group("schema")]
edit-schema:
    @${VISUAL:-${EDITOR:-nano}} information_models/dp_nbnl_equipment.schema.linkml.yml

# Show definition of the class identified by the provided CURIE
[group("schema")]
get-definition curie:
    yq '.classes.* | select(.class_uri == "{{curie}}")' information_models/dp_nbnl_equipment.schema.linkml.yml

# List all classes in the schema
[group("schema")]
list-classes:
    yq '.classes.* | key' information_models/dp_nbnl_equipment.schema.linkml.yml
...
generate-documentation: _post-process-linkml-schema
    ...
    poetry run python -m linkml_asciidoc_generator.main \
        "artifacts/information_models/dp_nbnl_equipment.schema.linkml.yml" \
...
generate-json-schema: _post-process-linkml-schema
    ...
    poetry run gen-json-schema \
        --not-closed \
        "artifacts/information_models/dp_nbnl_equipment.schema.linkml.yml" \
        > "artifacts/schemas/json_schema/dp_nbnl_equipment.json_schema.json"
    @echo "… OK."
    @echo
    @echo "Generated JSON Schema at: artifacts/schemas/json_schema/dp_nbnl_equipment.json_schema.json"
    @echo
...
generate-shacl-schema: _post-process-linkml-schema
    ...
    poetry run gen-shacl \
        --non-closed \
        "artifacts/information_models/dp_nbnl_equipment.schema.linkml.yml" \
        > "artifacts/schemas/shacl/dp_nbnl_equipment.shacl.ttl"
    @echo "… OK."
    @echo
    @echo "Generated SHACL at: artifacts/schemas/shacl/dp_nbnl_equipment.shacl.ttl"
    @echo
...
validate-example-data: generate-json-schema generate-shacl-schema generate-example-data
    ...
    for example_file in artifacts/examples/json/*.json; do \
        [ -f "$example_file" ] || continue; \
        poetry run check-jsonschema --schemafile "artifacts/schemas/json_schema/dp_nbnl_equipment.json_schema.json" $example_file; \
    done
    @echo "… OK."
    @echo
    ...
    for example_file in artifacts/examples/jsonld/*.jsonld; do \
        [ -f "$example_file" ] || continue; \
        poetry run pyshacl -s "artifacts/schemas/shacl/dp_nbnl_equipment.shacl.ttl" -df json-ld -f human $example_file; \
    done
    @echo "… OK."
    @echo
